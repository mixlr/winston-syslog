...
var Syslog = exports.Syslog = function (options) {
  ...
  this.cee         = options.cee         || false;
  this.message_key = options.message_key || 'message';
  this.meta_key    = options.meta_key    || null;
...
Syslog.prototype.log = function (level, msg, meta, callback) {
  ...
  if (this.cee || Object.keys(meta).length > 0) {
    if (this.meta_key) {
      data = {};
      data[this.message_key] = msg;
      if (Object.keys(meta).length > 0) {
        data[this.meta_key] = winston.clone(meta);
      }
    } else {
      data = winston.clone(meta || {});
      data[this.message_key] = msg;
    }
  ...
  syslogMsg = this.producer.produce({
    severity: level,
    date:     new Date(),
    message:  this.cee ? '@cee: ' + data : data
  }) + "\n";
